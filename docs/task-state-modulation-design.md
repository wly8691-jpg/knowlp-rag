...
> **v2 注记（2026-08-29 首轮回放复盘，影复核）**：
> 1. **串盘率指标升级**——二进制 top-8 串盘率对 0.7 阻尼不敏感（阻尼不足以把
>    残留节点挤出 top-8）；**rank 位次**（0.943→0.953）才是能看出 rule/policy
>    细微差异的粒度。后续评估串盘率二进制 + rank 位次并报，别只用二进制。
> 2. **π̂ 聚合方向**——per-node 增益目标（~419 维）太碎、无区分度（收敛到全局
>    -0.2）；v2 按**画像维度（tag/簇）聚合**增益目标，等攒够含 drift/rejected
>    信号的轨迹再训。
