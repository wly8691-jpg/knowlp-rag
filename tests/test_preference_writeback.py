"""#9 writeback tests: minimal scope + no dangling keys + only weight touched + dry-run zero writes."""
import json
...
def test_write_back_updates_touched_only(tmp_path, monkeypatch):
    ...
    assert after["weights"]["A||B"]["weight"] == 1.0, "dry-run must not write to disk"

    rep2 = wb.write_back()
    after2 = json.loads(gpath.read_text(encoding="utf-8"))
    w = after2["weights"]
    assert rep2["applied"] >= 2, "both chosen/rejected edges should change"
    assert w["A||B"]["weight"] > w["C||D"]["weight"], "chosen should rank above rejected"
    assert w["A||B"]["use_count"] == 3 and w["A||B"]["last_touch"] == 9.0, \
        "writeback must not touch use_count/last_touch"
    assert w["X||Y"]["weight"] == 0.5, "edges untouched by the buffer must not change"
    assert (tmp_path / "dual_graph.backup.json").exists(), "a backup should exist before writing"
...
def test_write_back_never_creates_dangling_keys(tmp_path, monkeypatch):
    ...
    # E||F no longer exists in the graph (gone after a rebuild) -> must be skipped, no new key
    buf = [_pair("s1", {"from": "A", "to": "B", "type": "pre"},
    ...
    assert "E||F" not in after["weights"], "must not create dangling weight keys"
...
def test_write_back_empty_buffer_is_noop(tmp_path, monkeypatch):
    ...
    rep = wb.write_back()
    assert "error" in rep, "empty buffer should return error instead of writing"
